...
class goToGoal:
  ...
  def __init__(self):
    ...
    self.emergency_subscriber = rospy.Subscriber("/turtlebot3/emergency_obstacle", Point,
  ...
  def go_to_goal(self):
    ...
    if self.within_bounds():
      ...
      rospy.loginfo("----------------------------")
      if self.current_waypoint < len(self.way_points):
        dist_to_goal.x = goal[0] - self.global_pos.x  # distance it needs to move in x direction
        dist_to_goal.y = goal[1] - self.global_pos.y  # distance it needs to move in y direction
        dist_to_goal.z = math.atan2(dist_to_goal.y, dist_to_goal.x) - self.global_ang  # orientation offset
      else:
  ...
  def drive_wheels(self, data):
    ...
    if data.x == 0 and data.y == 0 and data.z == 0:
      rospy.loginfo("Done with plan - don't move")
      ...
      rospy.loginfo("Emergency - Obstacle in way!!!!")
      x = self.obstacle_loc.x
      y = self.obstacle_loc.y
      twist = Twist()
      twist.angular.z = p_value_angular_emergency * (y - math.pi / 6) / (x ** 2) # 0.523 is 30 degrees; the closer you are, the more you turn
      self.vel_publisher.publish(twist)
      self.publish_rate.sleep()
      twist.angular.z = 0
      twist.linear.x = 0.1
      ...
      rospy.loginfo("Angle error: " + str(math.degrees(angle_error)) + " degrees")
      """
