...
lower= np.array([145, 10, 75],np.uint8) # Array (H,S,V) for the lower threshold bound of the HSV image
upper= np.array([180, 255, 255],np.uint8) # Array (H,S,V) for the upper threshold bound of the HSV image
error = np.array([13,30,30],np.uint8) # Array of error widths to create the upper and lower threshold bounds above.
...
def Init():
    ...
    global pub
    pub = rospy.Publisher("/turtlebot3/burger/ball_location", Point, queue_size=10)
